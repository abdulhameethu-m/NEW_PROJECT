...
const REVIEWED_NON_IDOR_CONTEXTS = [
  {
    pattern: /[\\/]modules[\\/]adminInfluencerCommerce[\\/]controller\.js$/,
    proof: "admin influencer-commerce controller is mounted only behind adminWorkspaceAuthRequired plus route-level influencerCommerce read/manage/payouts/settings permissions",
  },
  {
  ...
  {
    pattern: /[\\/]services[\\/](campaign-fee|campaign-scheduling|cancellation-policy|finance-config|guestWishlist|homepage-layout|influencer-commerce-config|platform-bootstrap|pricing-category|product-number|settlement-metrics|shipping-config|shipping-pricing|shipping-zone-config|shippingConfigAdmin)\.service\.js$/,
