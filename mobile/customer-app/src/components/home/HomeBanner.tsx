...
export const HomeBanner = ({ title, subtitle, imageUrl, ctaText, ctaUrl, isLoading }: HomeBannerProps) => {
  ...
  if (isLoading) {
    return (
      <View className="mx-4 my-2 h-40 rounded-2xl bg-slate-200 dark:bg-slate-800" />
    );
  }

  return (
    <View className="mx-4 my-2 h-40 rounded-2xl overflow-hidden bg-slate-900 relative justify-center px-6">
      {imageUrl && (
        <>
          <Image
            source={imageUrl}
