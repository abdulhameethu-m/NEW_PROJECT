...
const drawerSections = [
  ...
  },
  {
    title: "Shopping",
    items: [
      { label: "Coupons", path: "/dashboard/user/coupons", icon: TicketPercent },
    ...
    >
      <span
        className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${
          active ? "bg-white text-indigo-600 shadow-sm dark:bg-indigo-500/20" : "bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-300"
        }`}
      >
        ...
        <div className="border-b border-slate-100 p-4 dark:border-slate-800">
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3">
              <Avatar user={user} className="h-10 w-10 rounded-full border border-slate-200 dark:border-slate-800" textClassName="text-sm" />
              <div className="min-w-0">
                <p className="truncate text-sm font-bold text-slate-950 dark:text-white">{user?.name || "Customer"}</p>
                <p className="truncate text-[11px] text-slate-500 dark:text-slate-400">{user?.email || user?.phone}</p>
                <span className="mt-1 flex w-max items-center gap-1 rounded-full bg-emerald-50 px-2 py-0.5 text-[10px] font-bold text-emerald-600 dark:bg-emerald-950/30 dark:text-emerald-400">
            ...
            <Link
              to="/profile"
              onClick={onClose}
              className="flex w-full items-center justify-center gap-2 rounded-xl border border-slate-200 py-2 text-xs font-semibold text-indigo-600 transition hover:bg-indigo-50 dark:border-slate-800 dark:text-indigo-400 dark:hover:bg-indigo-500/10"
            >
