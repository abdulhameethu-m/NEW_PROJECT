...
export function normalizeCartPayload(payload) {
  const candidate =
    payload?.cart && Array.isArray(payload.cart.items)
      ? payload.cart
...
// getAvailableProductVariant removed. Inventory calculation is now exclusively handled by the backend Smart Cart Allocation Engine.
