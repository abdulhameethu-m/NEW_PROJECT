  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        // Use promise all to fetch what we have permission for
        // We will fetch getDashboard if we have basic access (we assume dashboard is accessible)
        tasks.push(getDashboard().catch(() => ({ data: null })));
        
        if (canReadAnalytics) {
          tasks.push(getAnalytics().catch(() => ({ data: null })));
          tasks.push(getDailyRevenue(chartDays).catch(() => ({ data: [] })));
        } else {
          tasks.push(Promise.resolve({ data: null }));
          tasks.push(Promise.resolve({ data: [] }));
        }

        const [dashboardRes, analyticsRes, dailyRevenueRes] = await Promise.all(tasks);
        if (!alive) return;
        
        if (dashboardRes.data) setDashboard(dashboardRes.data);
        if (analyticsRes.data) setAnalytics(analyticsRes.data);
        if (dailyRevenueRes.data) setDailyRevenue(Array.isArray(dailyRevenueRes.data) ? dailyRevenueRes.data : []);
      } catch (err) {
